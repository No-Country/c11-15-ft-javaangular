...
export class PetallService {
  ...
  getPetByPage(limit: number, offset: number) {
    return this.http.get<Mascota[]>(`${this.apiUrl}/products`, {
      params: { limit, offset }
    })
  }

  create(data: CreateMascota) {
    return this.http.post<Mascota>(`${this.apiUrl}/products`, data);
  }

  update(id: string, data: UpdateMascota) {
    return this.http.put<Mascota>(`${this.apiUrl}/products/${id}`, data)
  }
